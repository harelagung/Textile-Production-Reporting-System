        ...
        return $table
            ->columns([
                ...
                TextColumn::make("total_stock")
                    ...
                    ->state(function ($record) {
                        ...
                        return number_format($cache[$key]);
                    }),
            ...
            ->headerActions([
                ...
                Action::make("shift_pagi")
                    ...
                    ->button()
                    ->color(
                        fn($livewire) => ($livewire->tableFilters["shift_id"]["value"] ?? null) == 1
                            ? "primary"
                            : "gray",
                    )
                ...
                Action::make("shift_siang")
                    ...
                    ->button()
                    ->color(
                        fn($livewire) => ($livewire->tableFilters["shift_id"]["value"] ?? null) == 2
                            ? "primary"
                            : "gray",
                    )
                    ->action(function ($livewire) {
                ...
                Action::make("shift_malam")
                    ...
                    ->button()
                    ->color(
                        fn($livewire) => ($livewire->tableFilters["shift_id"]["value"] ?? null) == 3
                            ? "primary"
                            : "gray",
                    )
                    ->action(function ($livewire) {
                ...
                Action::make("semua_shift")
                    ->label("Semua Shift")
                    ->button()
                    ->color(fn($livewire) => empty($livewire->tableFilters["shift_id"]["value"]) ? "info" : "gray")
                    ->action(function ($livewire) {
                ...
                // Tambahkan setelah Action::make("semua_tanggal") di headerActions
                Action::make("restore_data")
                    ->label("Restore")
                    ->button()
                    ->color("success")
                    ->icon("heroicon-o-arrow-path")
                    ->modalHeading("Data yang Sudah Dihapus")
                    ->modalDescription("Daftar data yang sudah dihapus dan dapat dipulihkan")
                    ->modalContent(view("filament.custom.restore-reports"))
                    ->modalWidth("6xl")
                    ->modalFooterActions([])
