...
class ReportController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            // Konfigurasi effectivity parameters
            $effectivityConfig = [
                1 => ["base_value" => 165, "divisor" => 48],
                2 => ["base_value" => 185, "divisor" => 56],
                3 => ["base_value" => 165, "divisor" => 56],
                4 => ["base_value" => 185, "divisor" => 48],
                5 => ["base_value" => 185, "divisor" => 56],
                6 => ["base_value" => 185, "divisor" => 48],
            ];

            // Hitung effectivity
            $config = $effectivityConfig[$constructions->id] ?? null;
            $effectivity = 0;

            if ($config) {
                $denominator = ($config["base_value"] * 2.541 * 0.6 * $workhours) / $config["divisor"];
                $effectivity = ($lateStock / $denominator) * 100;
            }
